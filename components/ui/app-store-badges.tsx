...
export function AppStoreBadges({
  ...
  const innerClasses = cn(
    "rounded-[14px] px-3.5 py-2 xs:px-5 xs:py-2.5 flex items-center gap-2.5 xs:gap-3 transition-all duration-300",
    isLight
  ...
  const titleClasses = cn(
    "font-semibold text-base xs:text-lg leading-none",
    isLight ? "text-background" : "text-white"
  )
    ...
    <div className={cn("flex flex-wrap items-center justify-center gap-4", className)}>
      ...
      >
        <div className={innerClasses}>
          <svg className={cn("w-6 h-6 xs:w-7 h-7", iconClasses)} viewBox="0 0 24 24" fill="currentColor">
      ...
      >
        <div className={innerClasses}>
          <img src="/Google Play logo.svg" alt="Google Play" className="w-4.5 h-4.5 xs:w-5 xs:h-5 object-contain" />
          <div className="text-left">
